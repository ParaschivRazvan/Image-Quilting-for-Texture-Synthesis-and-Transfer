...
for k = 1 : N
    ...
    for x = 1:nrBlocuriX
        for y = 1:nrBlocuriY
            ...
            d = zeros(1, nrBlocuri);
            correspondence_error = zeros(1, nrBlocuri);
            ...
            for i = 1:nrBlocuri
                ...
                B = blocuri(:,:,:,i);
                correspondence_error(i) = sum(( double(B(:)) - double(fereastraModel(:)) ).^2 );
                
                if k == 1
                    erori(i) = alpha * d(i) + (1 - alpha) * correspondence_error(i);
                else
                    previous_synthesized_error = sum(( double(B(:)) - double(fereastraAnterior(:)) ).^2 );
                    erori(i) = alpha * (d(i) + previous_synthesized_error) + (1 - alpha) * correspondence_error(i);
                end
